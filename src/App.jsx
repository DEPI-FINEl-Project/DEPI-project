...
const TEMPLATES = [
  ...
  { id: 'developer', name: 'Developer', desc: 'Dark, terminal-inspired, monospace. Built for engineers.' },
  { id: 'studio', name: 'Studio', desc: 'Bold, colorful, oversized type. For designers & makers.' }
];

const normalizeUrl = (url) => (!url ? '' : /^https?:\/\//.test(url) ? url : `https://${url}`);
...
function Portfolio({ data, preview = false }) {
  ...
  return (
    <div className={`pf pf-${data.template} pf-${data.theme}`} style={{ '--pf-accent': data.accent }}>
      <header className="pf-nav">
        <span className="pf-brand">{prompt ? `~/${(data.name || 'me').toLowerCase().replace(/\s+/g, '-')}` : data.name}</span>
        <nav>
          <a href="#pf-about" onClick={a}>About</a>
          <a href="#pf-work" onClick={a}>Work</a>
          <a href="#pf-contact" onClick={a}>Contact</a>
        </nav>
        <a className="pf-nav-cta" href={data.github ? `https://github.com/${data.github}` : '#pf-contact'} onClick={data.github ? undefined : a} target={data.github ? '_blank' : undefined} rel="noreferrer">{data.github ? <><i className="fa-brands fa-github" /> GitHub</> : 'Hire me'}</a>
      </header>

      <section className="pf-hero" id="pf-top">
        <span className="pf-tag">{prompt ? '$ whoami' : `Available — ${data.role}`}</span>
        <h1>{prompt ? <><span className="pf-cursor">{data.name}</span></> : data.name}</h1>
        <p className="pf-role">{data.role}</p>
        <p className="pf-tagline">{data.tagline}</p>
        <div className="pf-hero-actions"><a href="#pf-work" onClick={a} className="pf-btn">View Work</a><a href={`mailto:${data.email}`} onClick={preview ? a : undefined} className="pf-btn ghost">Get in touch</a></div>
        {data.skills.length > 0 && <ul className="pf-skills">{data.skills.map((skill) => <li key={skill}>{skill}</li>)}</ul>}
      </section>
      ...
      <section className="pf-work" id="pf-work">
        <div className="pf-work-head"><h2>{prompt ? '// selected work' : 'Selected Work'}</h2><span>{data.projects.length} {data.projects.length === 1 ? 'project' : 'projects'}</span></div>
        <div className="pf-projects">{data.projects.map((project, i) => {
          const href = normalizeUrl(project.link);
          return <article className="pf-card" key={i}>
            <div className="pf-card-top"><span className="pf-card-no">{String(i + 1).padStart(2, '0')}</span></div>
            <h3>{href ? <a href={href} onClick={preview ? a : undefined} target="_blank" rel="noreferrer">{project.title}</a> : project.title}</h3>
            <p>{project.description}</p>
            {project.tags?.length > 0 && <div className="pf-card-tags">{project.tags.map((tag) => <span key={tag}>{tag}</span>)}</div>}
            {href && <a className="pf-card-cta" href={href} onClick={preview ? a : undefined} target="_blank" rel="noreferrer">{prompt ? 'open' : 'View Project'} <i className="fa-solid fa-arrow-right" /></a>}
          </article>;
        })}</div>
      </section>
      ...
      <section className="pf-contact" id="pf-contact">
        ...
        <p>{data.location ? `Based in ${data.location}. ` : ''}Currently open to new projects and collaborations.</p>
        <div className="pf-contact-links">
          <a className="pf-btn" href={`mailto:${data.email}`} onClick={preview ? a : undefined} title={data.email}><i className="fa-regular fa-envelope" /> Email</a>
          {data.website && <a className="pf-btn ghost" href={normalizeUrl(data.website)} onClick={preview ? a : undefined} target="_blank" rel="noreferrer"><i className="fa-solid fa-globe" /> Website</a>}
          {data.github && <a className="pf-btn ghost" href={`https://github.com/${data.github}`} onClick={preview ? a : undefined} target="_blank" rel="noreferrer"><i className="fa-brands fa-github" /> GitHub</a>}
        </div>
      </section>

      <footer className="pf-foot"><span>© {data.name}</span><span>Built with AutoPortfolio</span></footer>
    </div>
  );
}
...
function SiteNav({ active = 'showcase', cta = true }) {
  ...
  function logout() { close(); store.logout(); navigate('/'); }
  return (
    <header className={`site-nav${open ? ' open' : ''}`}>
      <Link className="brand" to="/" onClick={close}>AutoPortfolio</Link>
      <button className="nav-toggle" aria-label="Toggle menu" aria-expanded={open} onClick={() => setOpen(!open)}><i className={`fa-solid ${open ? 'fa-xmark' : 'fa-bars'}`} /></button>
      <nav onClick={close}>
        {user ? <>
          ...
          {hasGithub && <NavLink className={active === 'profile' ? 'active' : ''} to="/profile">Profile</NavLink>}
        </> : <>
      ...
      {user
        ? <div className="nav-actions"><span className="nav-user"><i className="fa-solid fa-circle-user" /> {user.name}</span><button className="dark-btn nav-logout" onClick={logout}><i className="fa-solid fa-arrow-right-from-bracket" /> Logout</button></div>
        : cta && <div className="nav-actions"><Link to="/login" onClick={close}>Sign In</Link><Link className="dark-btn" to="/signup" onClick={close}>Get Started</Link></div>}
    </header>
  );
}
...
function Builder() {
  ...
  function importGithub() { if (github?.profile) setData((d) => ({ ...d, ...portfolioSeed(github) })); }
  function addSkill(value) { const s = value.trim(); if (s && !data.skills.includes(s)) set('skills')([...data.skills, s]); }
  function removeSkill(skill) { set('skills')(data.skills.filter((s) => s !== skill)); }
  function setProject(i, key, value) { set('projects')(data.projects.map((p, idx) => idx === i ? { ...p, [key]: value } : p)); }
    ...
    <main className="builder-shell">
      <aside className="builder-panel">
        <SkillField label="Template">
          <div className="tpl-switch">{TEMPLATES.map((t) => <button key={t.id} className={data.template === t.id ? 'active' : ''} onClick={() => set('template')(t.id)}>{t.name}</button>)}</div>
        </SkillField>

        <SkillField label="Theme">
          <div className="seg">{['light', 'dark'].map((mode) => <button key={mode} className={data.theme === mode ? 'active' : ''} onClick={() => set('theme')(mode)}>{mode === 'light' ? '☼ Light' : '☾ Dark'}</button>)}</div>
        </SkillField>

        <SkillField label="Accent color">
          <div className="accent-row">{ACCENTS.map((c) => <button key={c} className={`accent-dot${data.accent === c ? ' active' : ''}`} style={{ background: c }} onClick={() => set('accent')(c)} aria-label={c} />)}<label className="accent-custom" style={{ background: data.accent }}><input type="color" value={data.accent} onChange={field('accent')} /></label></div>
        </SkillField>
        ...
        <input className="chip-input" placeholder="Type a skill, press Enter" onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); addSkill(e.target.value); e.target.value = ''; } }} />

        <div className="panel-h-row"><h3 className="panel-h">Projects</h3><button className="add-btn" onClick={addProject}><i className="fa-solid fa-plus" /> Add</button></div>
        {data.projects.map((project, i) => <div className="proj-edit" key={i}>
      ...
      <section className="builder-preview">
        <div className="preview-stage">
          <div className="stage-bar"><span /><span /><span /><b>your-portfolio.com</b></div>
          <div className="stage-scroll"><Portfolio data={data} preview /></div>
        </div>
      </section>
    </main>

    {saved && <div className="saving-toast"><i className="fa-solid fa-check" /> Saved</div>}
  </div>;
}
...
function RepoGrid({ repos, username }) {
  if (!repos.length) return <section className="works"><div className="works-heading"><div><h2>Repositories</h2><p>No public repositories found for @{username}.</p></div></div></section>;
  return <section className="works"><div className="works-heading"><div><h2>Repositories</h2><p>Live from GitHub, sorted by popularity.</p></div><a href={`https://github.com/${username}?tab=repositories`} target="_blank" rel="noreferrer">View all on GitHub <i className="fa-solid fa-arrow-right" /></a></div><div className="repo-grid">{repos.map((repo) => <article className="repo-card" key={repo.id}><div className="repo-top"><i className="fa-solid fa-book-bookmark" /><a href={repo.url} target="_blank" rel="noreferrer">{repo.name}</a></div><p>{repo.description || 'No description provided.'}</p>{repo.topics.length > 0 && <div className="repo-topics">{repo.topics.slice(0, 4).map((topic) => <span key={topic}>{topic}</span>)}</div>}<div className="repo-meta">{repo.language && <span><i className="fa-solid fa-circle" /> {repo.language}</span>}<span><i className="fa-regular fa-star" /> {repo.stars}</span><span><i className="fa-solid fa-code-fork" /> {repo.forks}</span>{repo.homepage && <a href={repo.homepage} target="_blank" rel="noreferrer"><i className="fa-solid fa-arrow-up-right-from-square" /> Live</a>}</div></article>)}</div></section>;
}
...
function Profile() {
  ...
  const languages = useMemo(() => {
    ...
    const counts = {};
    github.repos.forEach((repo) => { if (repo.language) counts[repo.language] = (counts[repo.language] || 0) + 1; });
    return Object.entries(counts).sort((a, b) => b[1] - a[1]).map(([lang]) => lang).slice(0, 8);
  }, [github]);
  const totalStars = useMemo(() => github?.repos?.reduce((sum, repo) => sum + repo.stars, 0) || 0, [github]);
  function disconnect() { store.clearGithub(); navigate('/dashboard'); }
  if (!github || !github.profile) return <PageShell active="profile"><section className="inner-heading"><span className="pill">Profile</span><h1>No GitHub account connected</h1><p>Connect your GitHub account to generate your profile automatically.</p></section><button className="dark-btn big" onClick={disconnect}><i className="fa-brands fa-github" /> Connect GitHub</button></PageShell>;
  const { profile, repos } = github;
  return <div className="framed profile-frame"><SiteNav active="profile" /><section className="gh-profile"><div className="gh-profile-card"><img className="gh-avatar" src={profile.avatar} alt={profile.name} width="160" height="160" /><div className="gh-identity"><h1>{profile.name}</h1><a className="gh-login" href={profile.url} target="_blank" rel="noreferrer">@{profile.login}</a>{profile.bio && <p className="gh-bio">{profile.bio}</p>}<div className="gh-tags">{profile.company && <span><i className="fa-solid fa-building" /> {profile.company}</span>}{profile.location && <span><i className="fa-solid fa-location-dot" /> {profile.location}</span>}{profile.blog && <a href={/^https?:\/\//.test(profile.blog) ? profile.blog : `https://${profile.blog}`} target="_blank" rel="noreferrer"><i className="fa-solid fa-link" /> Website</a>}{profile.twitter && <a href={`https://twitter.com/${profile.twitter}`} target="_blank" rel="noreferrer"><i className="fa-brands fa-x-twitter" /> @{profile.twitter}</a>}</div><div className="gh-actions"><a className="dark-btn" href={profile.url} target="_blank" rel="noreferrer"><i className="fa-brands fa-github" /> View on GitHub</a><button className="gh-disconnect" onClick={disconnect}><i className="fa-solid fa-arrow-right-from-bracket" /> Disconnect</button></div></div></div><div className="gh-stats"><div><strong>{profile.publicRepos}</strong><span>Repositories</span></div><div><strong>{profile.followers}</strong><span>Followers</span></div><div><strong>{profile.following}</strong><span>Following</span></div><div><strong>{totalStars}</strong><span>Stars earned</span></div></div>{languages.length > 0 && <div className="gh-langs"><h2>Top Languages</h2><div>{languages.map((lang) => <b key={lang}>{lang}</b>)}</div></div>}</section><RepoGrid repos={repos} username={profile.login} /><Footer /></div>;
}
